library(pheatmap)
setwd("D:\\桌面文件夹\\3\\17 survival state and heatmap")
...
bioRiskPlot <- function(inputFile = null, project = null) {
    ...
    plot(line,
        ...
        ylab = "Risk score",
        col = c(rep("orange", lowLength), rep("blue", highLength))
    )
    abline(h = lowMax, v = lowLength, lty = 2)
    legend("topleft", c("High risk", "Low Risk"), bty = "n", pch = 19, col = c("blue", "orange"), cex = 1.2)
    dev.off()

    color <- as.vector(rt$fustat)
    color[color == 1] <- "blue"
    color[color == 0] <- "orange"
    ...
    legend("topleft", c("Dead", "Alive"), bty = "n", pch = 19, col = c("blue", "orange"), cex = 1.2)
    abline(v = lowLength, lty = 2)
    dev.off()

    ann_colors <- list()
    bioCol <- c("orange", "blue")
    ...
    pheatmap(rt1,
        ...
        show_colnames = F,
        scale = "row",
        color = colorRampPalette(c(rep("orange", 3.5), "white", rep("blue", 3.5)))(50),
        fontsize_col = 3,
        fontsize = 7,
        fontsize_row = 8
    )
